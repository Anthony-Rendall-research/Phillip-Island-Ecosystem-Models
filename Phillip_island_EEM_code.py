...
class EEM_reintro:
    def __init__(self, ensemble, reintro_id, control_id, control_amount, T = 50):#number of years over which predictions occur
...
def generate_phillip_island_ensemble(fname='PI_EEM', rep=10000, response = [], rem_node = [], final_removed=[], return_num_unstable=False, j=''):
    ...
    if return_num_unstable:
        ...
        fname = fname + '_' + str(rep) + str(j) + '.pkl'
...
if __name__ == "__main__":
    ...
    generate_param_histograms = False
...
#    reintro = EEM_reintro(ensemble, reintro_sp, control, control_level)
    ...
    if run_standard_analysis:
        ...
        for j in range(num_repeats):
            ...
            #generate_phillip_island_ensemble(rep=reps, response = [[7, True], [10, True]], rem_node = [0],final_removed = [0, 16])
            ensemble = load_object('PI_EEM_10000{0}.pkl'.format(j))
            control = [1]  # controlling cats
            control_level = 1.5 # control cats to 50% of current
            reintro_sp = None  # reintroduce bandicoots
            reintro = EEM_reintro(ensemble, reintro_sp, control, control_level)
            ...
            uncertainty_list2.append(results)
        uncertainty_array2 = np.array(uncertainty_list2)
        uncertainty_DF2 = pd.DataFrame(uncertainty_array2)
        uncertainty_DF2.to_csv('Cat_suppression_50yrs_cats_1_5_2.csv', index=False)
            #plt.hist(bandicoot_data, bins= 'auto')
            #plt.title("Frequency of increase: {}".format((int(mean_increase * 1000)) / 1000))
            #plt.show()

            # Print results out
            #np.savetxt('Final_results_Bandicoot_introduction_2yrs_cat_supp_0_5c.csv', results, delimiter = ',')
    ...
    if uncerainty_across_param_sets_flag:
        ...
        for j in range(num_repeats):
            ...
            response = [[7, True], [10, True]]  # these increase when node is removed
            final_removed = [0]  # return an ensemble without these species
            generate_phillip_island_ensemble(rep=reps, response = [[7, True], [10, True]], rem_node = [0],final_removed = [0, 16], j=j)
            ensemble = load_object('PI_EEM_10000{0}.pkl'.format(j))
            control = [1]  # controlling cats
        ...
        uncertainty_DF.to_csv('uncertainty_across_param_sets_50yrs.csv', index=False)
